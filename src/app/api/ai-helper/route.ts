...
async function generateAIResponse(
  message: string
): Promise<{ response: string; serviceSuggestion?: ServiceSuggestion }> {
  ...
  for (const [serviceKey, keywords] of Object.entries(serviceKeywords)) {
    ...
    if (hasKeyword) {
      const service =
        serviceSuggestions[serviceKey as keyof typeof serviceSuggestions];
      return {
  ...
  return {
    response:
      "Entiendo tu problema. Para darte la mejor recomendación, ¿podrías ser más específico sobre qué tipo de servicio necesitas? Por ejemplo: problemas de agua, electricidad, aire acondicionado, etc.",
  };
}
