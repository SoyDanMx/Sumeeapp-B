...
export default function LeadStatusClient({ initialLead }: LeadStatusClientProps) {
          ...
          <div className="text-center mb-8">
            <div className={`w-20 h-20 ${statusInfo.bgColor} rounded-full flex items-center justify-center mx-auto mb-4`}>
              <FontAwesomeIcon 
                ...
                className={`text-3xl ${statusInfo.color} ${lead.estado === 'buscando' ? 'animate-spin' : ''}`} 
              />
            </div>
            <h2 className="text-3xl font-bold text-gray-900 mb-2">{statusInfo.title}</h2>
          ...
          <div className="bg-gray-50 rounded-lg p-6 mb-8">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="flex items-center space-x-3">
                ...
                <span><strong>Cliente:</strong> {lead.nombre_cliente}</span>
              </div>
              <div className="flex items-center space-x-3">
                <FontAwesomeIcon icon={faMapMarkerAlt} className="text-red-600" />
                <span><strong>Ubicación:</strong> CDMX</span>
              </div>
              <div className="flex items-center space-x-3">
                <FontAwesomeIcon icon={faClock} className="text-gray-600" />
                <span><strong>Fecha:</strong> {new Date(lead.fecha_creacion).toLocaleDateString()}</span>
              </div>
              {lead.whatsapp && (
                <div className="flex items-center space-x-3">
            ...
            {lead.imagen_url && (
              <div className="mt-4">
                <h4 className="font-semibold text-gray-900 mb-2">Imagen Adjunta</h4>
                <img 
                  src={lead.imagen_url} 
            ...
            <div className="bg-green-50 rounded-lg p-6 mb-8">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  <h4 className="text-xl font-bold text-gray-900">{lead.profesional_asignado?.full_name}</h4>
                  <p className="text-gray-600">{lead.profesional_asignado?.profession}</p>
                    ...
                    <div className="flex items-center space-x-1 mt-1">
                        {Array.from({ length: 5 }, (_, i) => (
                          <FontAwesomeIcon 
                            ...
                            icon={faStar} 
                            className={`text-sm ${
                              i < Math.floor(lead.profesional_asignado?.calificacion_promedio || 0)
                                ? 'text-yellow-400'
                                : 'text-gray-300'
                            }`}
                          />
                        ...
                        <span className="text-sm text-gray-600 ml-1">
                          ({lead.profesional_asignado?.calificacion_promedio?.toFixed(1)})
                        </span>
                    </div>
                  )}
                </div>
