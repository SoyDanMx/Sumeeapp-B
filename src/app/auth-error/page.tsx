...
import { faExclamationTriangle, faRefresh, faHome, faCheckCircle, faSpinner } from '@fortawesome/free-solid-svg-icons';
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
        <div className="mb-6">
          <FontAwesomeIcon 
            icon={isClearing ? faSpinner : isCleared ? faCheckCircle : faExclamationTriangle}
            className={`text-6xl mb-4 ${isClearing ? 'text-blue-500 animate-spin' : isCleared ? 'text-green-500' : 'text-yellow-500'}`}
          />
          <h1 className="text-2xl font-bold text-gray-900 mb-2">
            {isClearing ? 'Limpiando Sesión...' : isCleared ? 'Sesión Limpiada' : 'Error de Autenticación'}
          </h1>
        ...
        {!isClearing && !isCleared && (
          <div className="text-left mb-6 space-y-2 text-sm text-gray-600">
            ...
            <p>• El error "Auth session missing" indica una sesión corrupta</p>
          </div>
        )}
        ...
        <div className="mt-6 text-xs text-gray-500">
          ...
          <p className="mt-2 text-blue-600">
            <a href="https://wa.me/525636741156?text=Hola,%20tengo%20un%20problema%20con%20la%20autenticación%20en%20Sumee%20App" 
               target="_blank" 
