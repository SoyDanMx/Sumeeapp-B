...
interface SyscomStyleFiltersProps {
...
export default function SyscomStyleFilters({
  products,
  categoryId,
  filters,
  onFiltersChange,
  onClearFilters,
}: SyscomStyleFiltersProps) {
  ...
  const activeFilterChips = useMemo(() => {
    ...
    if (filters.rama && hierarchy) {
      ...
      if (rama) {
        chips.push({
          label: rama.name,
          onRemove: () => onFiltersChange({ ...filters, rama: null, subrama: null }),
        });
      }
    }
    ...
    if (filters.subrama && currentRama) {
      ...
      if (subrama) {
        chips.push({
          label: subrama.name,
          onRemove: () => onFiltersChange({ ...filters, subrama: null }),
        });
      }
    }
    ...
    if (filters.priceRange.min !== null || filters.priceRange.max !== null) {
      ...
      chips.push({
        ...
        onRemove: () => onFiltersChange({ ...filters, priceRange: { min: null, max: null } }),
      });
    }
            ...
            {expandedSections.hierarchy && (
              <div className="px-4 pb-4 space-y-1">
                {!filters.rama ? (
                  ...
                  <>
                    <button
                      onClick={() => onFiltersChange({ ...filters, rama: null, subrama: null })}
