-- =====================================================
-- FIX COMPLETO: Trigger handle_new_user (SIN columna city)
...
-- PASO 3: Crear función COMPLETA y CORREGIDA (SIN city)
...
BEGIN
  ...
  IF v_registration_type = 'professional' OR v_profession IS NOT NULL THEN
    -- Insertar como PROFESIONAL (SIN city)
  ...
  ELSE
    -- Insertar como CLIENTE (SIN city)
...
BEGIN
  ...
  IF v_trigger_count > 0 AND v_function_count > 0 THEN
    RAISE NOTICE '✅ Trigger y función creados exitosamente (SIN columna city)';
