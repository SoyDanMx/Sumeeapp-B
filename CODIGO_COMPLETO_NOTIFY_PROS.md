...
serve(async (req) => {
  ...
  try {
    ...
    const disciplina = lead.disciplina_ia || lead.servicio_solicitado || lead.servicio || "General";
    ...
    if (lead.ubicacion_lat && lead.ubicacion_lng) {
      query = query.or(`profession.ilike.%${disciplina}%,areas_servicio.cs.{${disciplina}}`);
    } else {
      query = query.or(`profession.ilike.%${disciplina}%,areas_servicio.cs.{${disciplina}}`);
    }
    ...
    if (queryError) {
      ...
      const filtered = allPros.filter((p: any) => {
        ...
        return profession.includes(disciplinaLower) || areas.some((a: string) => a.includes(disciplinaLower));
      });
    ...
    if (lead.ubicacion_lat && lead.ubicacion_lng) {
      finalProfessionals = professionals.filter((p: any) => {
        ...
        const distance = calculateDistance(lead.ubicacion_lat!, lead.ubicacion_lng!, p.ubicacion_lat, p.ubicacion_lng);
