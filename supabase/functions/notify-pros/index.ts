...
// Configuración de Resend (cambiar por tu API key)
...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Que tengan la profesión o área de servicio que coincida
    // 2. Que estén dentro de un radio de 50km (usando PostGIS si está disponible)
    let query = supabase
      .from("profiles")
    ...
    // Buscar en profession o areas_servicio
    ...
    if (lead.ubicacion_lat && lead.ubicacion_lng) {
      // Query con PostGIS (asumiendo que tienes la extensión habilitada)
    ...
    if (queryError) {
      ...
      const { data: allPros } = await supabase
        .from("profiles")
        .select("email, full_name, profession")
        .eq("role", "profesional")
        .not("email", "is", null)
        .limit(100);
  ...
  const emailPromises = professionals.map(async (pro) => {
    try {
      ...
      const response = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "Sumee <notificaciones@sumeeapp.com>", // Cambiar por tu dominio verificado
          to: pro.email,
